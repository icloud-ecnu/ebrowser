...
public class ContentViewCore implements AccessibilityStateChangeListener, DisplayAndroidObserver,
	...
	Log.d(TAG, "flingViewport-----velocityX: %s velocityY: %s", velocityX,velocityY);
	Log.w(TAG, "flingViewport-----velocityX: %s velocityY: %s", velocityX,velocityY);
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
            ...
            switch (event.getAction()) {
	            ...
	            Log.w(TAG, "ContentViewCore:"+MotionEvent.ACTION_SCROLL);
                    if (mNativeContentViewCore == 0) return false;
...
Log.w(TAG, "ContentViewCore-dx: %s, dy: %s, time: %s",dxPix,dyPix,time);
        if (mNativeContentViewCore == 0) return;
	...
	Log.w(TAG, "ContentViewCore-dx: %s, dy: %s",dxPix,dyPix);
    }
